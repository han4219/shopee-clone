...
const purchaseTabs = [
  {
    label: 'tất cả',
    status: purchasesStatus.ALL_PURCHASES
  },
  {
    label: 'chờ xác nhận',
    status: purchasesStatus.ON_WAITING_CONFIRMATION
  },
  {
    label: 'đang lấy hàng',
    status: purchasesStatus.ON_PROGRESS
  },
  {
    label: 'đang vận chuyển',
    status: purchasesStatus.ON_SHIPPING
  },
  {
    label: 'đã giao',
...
export default function PurchaseOrder() {
  ...
  const { data: purchaseData } = useQuery({
    queryKey: ['get-purchase-history', { purchaseStatus: status }],
    queryFn: () => purchaseApi.getPurchases(status as purchasesStatus)
  })
  const purchaseHistory = purchaseData?.data.data
    ...
    <div>
      <div className='sticky top-0 overflow-auto'>
        <div className='min-w-[700px]'>
          <div className='grid grid-cols-6 rounded-sm bg-white'>
            {purchaseTabs.map((tab) => (
              <div key={tab.status} className='col-span-1'>
                <Link
                  to={{
                    pathname: '/user/' + path.purchaseOrder,
          ...
          <div>
            {purchaseHistory && purchaseHistory?.length > 0 ? (
              purchaseHistory.map((purchase) => (
                <div key={purchase._id} className='mt-4 rounded-sm shadow'>
                    ...
                    <div className='bg-white px-2'>
                      <div className='flex items-center gap-3 border-b-2 border-dotted border-b-gray-400 bg-white py-6 px-5 pb-3'>
                        ...
                        <div className='flex-shrink-0'>
                          ...
                          <span className='ml-2 text-orange'>₫{formatProductPrice(purchase.price)}</span>
                        </div>
                      </div>
                    </div>
              ...
              <div className='mt-6 flex h-[600px] w-full flex-col items-center justify-center bg-white'>
                <div>
                  <img
                ...
                <div className='text-xl'>Chưa có đơn hàng</div>
              </div>
            )}
          </div>
        </div>
